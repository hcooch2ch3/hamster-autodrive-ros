...
class HamsterCameraNode(Node):
    def __init__(self):
        ...
        self.processed_image_publisher = self.create_publisher(Image, '/camera/image_processed', 10)
        ...
        self.get_logger().info(f'Hamster Camera Node started with URL: {self.camera_url}')
    ...
    def detect_objects(self, image, contours):
        ...
        for contour in contours:
            ...
            if area > threshold:
                ...
                # 객체 정보 표시
                cv2.putText(image, f'Obj{object_count}({int(area)})', 
        ...
        # 전체 객체 수 표시
        cv2.putText(image, f'Objects: {object_count}', 
    ...
    def object_following_control(self, detection_results, image_shape):
        ...
        self.get_logger().info(f'Following object: error_x={error_x}, area={largest_object["area"]}')
