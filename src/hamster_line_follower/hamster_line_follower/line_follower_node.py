...
class LineFollowerNode(Node):
    def __init__(self):
        ...
        self.processed_image_publisher = self.create_publisher(Image, '/line_follower/processed_image', 10)
    ...
    def detect_line(self, image):
        ...
        if self.get_parameter('white_line_detection').value:
            if self.get_parameter('adaptive_threshold').value:
                ...
                _, white_mask = cv2.threshold(masked_gray, brightness_threshold, 255, cv2.THRESH_BINARY)
        ...
        cv2.rectangle(processed_image, (0, roi_y), (width, roi_y + roi_height), (0, 255, 0), 2)
        ...
        if lines is not None:
            ...
            for x1, y1, x2, y2 in valid_lines:
                ...
                cv2.putText(processed_image, f'{angle:.1f}°', 
                           ((x1+x2)//2, (y1+y2)//2), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)
            ...
            if valid_lines:
                ...
                # Draw line center
                cv2.circle(processed_image, (line_center, roi_y + roi_height//2), 10, (255, 0, 0), -1)
        
        # Draw image center reference
        image_center = width // 2
        cv2.line(processed_image, (image_center, 0), (image_center, height), (255, 255, 0), 2)
    ...
    def follow_line(self, line_center, image_width):
        ...
        self.get_logger().info(f'Line following: error={error}, angular_z={cmd.angular.z:.3f}')
